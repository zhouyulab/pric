...
class eQTL(object):
    def __init__(self, line):
        chrom, start,end, Type, gene_name, source, table, refID = line.strip().split("\t")
        self.chrom = chrom
    ...
    def get_eQTL_pos(self, gene_start, gene_end):
        ...
        d1 = self.start - gene_start
        d2 = self.start - gene_end
    ...
    def get_bins(self, flank, bin, GeneInfo, gene_name):
        ...
        if relative_position == "up":
            flank_start = max(self.start-flank, 0)
            flank_end = self.start
        elif relative_position == "down":
            flank_start = self.start
            flank_end = self.start + flank
...
def find_eQTL_bin_chimeric(chrom, bin, gene_name, GeneInfo, ChimericInfo):
    gene_chrom, gene_start, gene_end, gene_strand = GeneInfo[gene_name]
    if gene_chrom not in ChimericInfo:
        ChimericInfo = []
    else:
...
def main():
    ...
    with open(f_out, "w") as fo:
        for key in eQTLinfo:
            chrom, start, end, Type, gene_name = key
            site = int(start)
            e_info = eQTLinfo[key]
            ...
            outline = [chrom, site, gene_name, e_info.Type] + eQTL_bin_bum
