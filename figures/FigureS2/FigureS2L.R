...
f_in = "./pRIC/results/analysis/A_WT_merge.ChimericStatis.txt"
f_outtxt = "./figures/results/FigureS2/FigureS2L/FigureS2L.txt"
f_outpdf = "./figures/results/FigureS2/FigureS2L/FigureS2L.pdf"

df = read_delim(f_in, "\t", col_names = FALSE)
colnames(df) <- c("Group", "ReadNum")
df$Total = sum(df$ReadNum)
df = df %>% transform(Percent=round(ReadNum*100/Total, 2)) %>% transform(label=paste(Percent, "%", sep = ""))
