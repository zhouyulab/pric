...
load_inter <- function(f_inter, Rep){
  ...
  df = df %>% separate("Gene1", into = c("GeneType1", "GeneName1"), sep = "[|]") %>% filter(GeneType1=="protein") %>%
    separate("Gene2", into = c("GeneType2", "GeneName2"), sep = "[|]") %>% filter(GeneType2=="protein")
  df = df %>% dplyr::select(c("GeneName1", "GeneName2", "ReadNum"))
...
load_intral <- function(f_intral, Rep){
  ...
  df = df %>% separate("GeneName", into = c("GeneType", "GeneName"), sep = "[|]") %>% filter(GeneType=="protein")
  df = df %>% dplyr::select(c("GeneName", "ReadNum"))
...
f_rep1_inter = "./pRIC/results/analysis/AD_WT_Rep1.inter.Chimeric.txt"
f_rep1_intral = "./pRIC/results/analysis/AD_WT_Rep1.intra.Chimeric.txt"
f_rep2_inter = "./pRIC/results/analysis/AD_WT_Rep2.inter.Chimeric.txt"
f_rep2_intral = "./pRIC/results/analysis/AD_WT_Rep2.intra.Chimeric.txt"

f_outtxt = "./figures/results/FigureS2/FigureS2C/FigureS2C.txt"
f_outpdf = "./figures/results/FigureS2/FigureS2C/FigureS2C.pdf"
...
p_inter = plot_cor(df.inter, "WT", "inter")
p_inter
